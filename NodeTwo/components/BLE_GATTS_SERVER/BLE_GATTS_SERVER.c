...
/// @brief queue cua mqtt
extern xQueueHandle queue_ble_mqtt;
extern xQueueHandle queue_subcribe_topic;
extern xQueueHandle queue_publish_topic;
...
//cấu trúc để chuẩn bị các dữ liệu.
...
// dùng để quản lý một cáu trúc Profile bên trong BLE
struct gatts_profile_inst {
    ...
    esp_gatt_srvc_id_t service_id; // thực ra nó chính là Service UUID và bao gồm các trạng thái khác.
    uint16_t char_handle;
    esp_bt_uuid_t char_uuid; //
    esp_gatt_perm_t perm; // là permission của một attribute có trong db ( có thể áp dụng cho service và characteristic)
    esp_gatt_char_prop_t property; // đây chỉ là một Properties (hay có thể nói là permission) của một characteristic cụ thể (chỉ có thể áp dụng cho characteristic )
...
static const uint16_t CHAR_SUBCRIBE_WR             = 0xFF00;
static const uint16_t CHAR_PUBLISH_WR              = 0xFF01;
static const uint16_t CHAR_MESSAGE_WR              = 0xFF02;
// 4 service cóp sẵn
...
//------------Char Propertis / Permission -------------//
static const uint8_t char_prop_read_write          = ESP_GATT_CHAR_PROP_BIT_WRITE | ESP_GATT_CHAR_PROP_BIT_READ;
//-----------------------------------------------------//

//-------------------Char Value------------------------//
// giá trị được lưu trữ trong database theo từng Attribute
static const uint8_t char_subcribe_name[]   = "Subcribe topic";
static const uint8_t char_publish_name[]   = "Publish topic";
static const uint8_t char_message_name[]   = "Sending message";
//-----------------------------------------------------//

//-------------------Char Value------------------------//
static char char_subcribe_value[100];
static char char_publish_value[100];
static char char_message_value[200];
//-----------------------------------------------------//
...
static const esp_gatts_attr_db_t gatt_db[HRS_IDX_NB] =
{
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&CHAR_SUBCRIBE_WR, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE ,
     SUBCRIBE_CHAR_VAL_LEN, sizeof(char_subcribe_value), (uint8_t *)&char_subcribe_value}},

    [IDX_CHAR_SUBCRIBE_CFG] =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_user_description, ESP_GATT_PERM_READ,
     sizeof(char_subcribe_name), sizeof(char_subcribe_name) , (uint8_t *)&char_subcribe_name}},
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&CHAR_PUBLISH_WR, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE ,
     PUBLISH_CHAR_VAL_LEN, sizeof(char_publish_value), (uint8_t *)&char_publish_value}},

    [IDX_CHAR_PUBLISH_CFG] =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_user_description, ESP_GATT_PERM_READ,
     sizeof(char_publish_name), sizeof(char_publish_name) , (uint8_t *)&char_publish_name}},
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&CHAR_MESSAGE_WR, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE ,
     MESSAGE_CHAR_VAL_LEN, sizeof(char_message_value), (uint8_t *)&char_message_value}},

    [IDX_CHAR_MESSAGE_CFG] =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_user_description, ESP_GATT_PERM_READ,
     sizeof(char_message_name), sizeof(char_message_name) , (uint8_t *)&char_message_name}},
};
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    switch (event) {
        ...
        case ESP_GATTS_READ_EVT:
            ...
            if(gatt_db_handle_table[IDX_CHAR_SUBCRIBE_VAL] == param->read.handle) {
                ESP_LOGI(EXAMPLE_TAG, "(2) ***** subcribed topic : %s  ***** \n",char_subcribe_value);
            }
            if(gatt_db_handle_table[IDX_CHAR_PUBLISH_VAL] == param->read.handle) {
               ESP_LOGI(EXAMPLE_TAG, "(2) ***** Published topic : %s  ***** \n",char_publish_value);
            }
            if(gatt_db_handle_table[IDX_CHAR_MESSAGE_VAL] == param->read.handle) {
                ESP_LOGI(EXAMPLE_TAG, "(4) ***** Message sent : %s  ***** \n",char_message_value);
            }
       	    break;
        case ESP_GATTS_WRITE_EVT:
            if (!param->write.is_prep){
                if(gatt_db_handle_table[IDX_CHAR_SUBCRIBE_VAL] == param->write.handle)
                {
                    BaseType_t ret;
                    ret = xQueueSend(queue_subcribe_topic,(char*)param->write.value,(TickType_t)10);
                    if (ret != pdPASS)
                    {
                        ESP_LOGE(EXAMPLE_TAG, "error sending message to queue");
                    }
                    else
                    {
                        ESP_LOGI(EXAMPLE_TAG, "sending SUBCRIBE topic to queue successfully");
                    }
                }
                if(gatt_db_handle_table[IDX_CHAR_PUBLISH_VAL] == param->write.handle)
                {
                    BaseType_t ret;
                    ret = xQueueSend(queue_publish_topic,(char*)param->write.value,(TickType_t)10);
                    if (ret != pdPASS)
                    {
                        ESP_LOGE(EXAMPLE_TAG, "error sending message to queue");
                    }
                    ...
                    {
                        ESP_LOGI(EXAMPLE_TAG, "sending PUBLISH topic to queue successfully");
                    }
                }
                
                if(gatt_db_handle_table[IDX_CHAR_MESSAGE_VAL] == param->write.handle)
                {
                    BaseType_t ret;
                    ret = xQueueSend(queue_ble_mqtt,(char*)param->write.value,(TickType_t)10);
                    if (ret != pdPASS)
                    {
                        ESP_LOGE(EXAMPLE_TAG, "error sending message to queue");
                    }
                    ...
                    {
                        ESP_LOGI(EXAMPLE_TAG, "sending MESSAGE to queue successfully");
                    }
                }
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT:{
            if (param->add_attr_tab.status != ESP_GATT_OK){
                ...
                // khởi tạo các handle trong att database
...
void ble_gatt_init(void)
{
    ...
    // đăng kí sự kiện ngắt , handler cho GATT
    ...
    //đăng kí sự kiện ngắt , handler cho GAP
